...
class Kn extends Xlsx{
	...
	function main(){
		...
		if (!empty($this->_view->data)){
			...
			foreach ($this->_view->data as $uid=>$User){
				...
				foreach ($User['learning_plans'] as $path_id=>$LP){
					$PHPXlsx->setActiveSheetIndex(0)
						...
						->setCellValue('G'.$line, '')//Starting level
						->setCellValue('H'.$line, '')//Current level
						->setCellValue('I'.$line, $LP['path_name'])//Booked program
						->setCellValue('J'.$line, $LP['user_lp_date_end_validity'])
						->setCellValue('K'.$line, empty($LP['courses']) ? 0 : count($LP['courses']))//Amount of modules in learning plan
						->setCellValue('L'.$line, empty($LP['user_lp_completed']) ? 'Incomplete' : 'Complete')//Completion status of learning plan
						->setCellValue('M'.$line, '')//On track
					;
					
					$elearnings = $microlearnings = $sessions = $lsat = array();
					$globalTime = $iCol = 0;
					$lastAccess = null;
					$courseColumns = array('N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z',
					...
					if (!empty($lsat)){
						foreach ($lsat as $course_id=>$Course){
							$PHPXlsx->setActiveSheetIndex(0)
								->setCellValue($courseColumns[$iCol].$line, $Course['course_name'])
								->setCellValue($courseColumns[$iCol+1].$line, $Course['user_course_status'] ? 'Complete' : 'Incomplete')
							;
							$iCol+=2;
						}
					}
						...
						if (!empty($microlearnings)){
							...
							$PHPXlsx->setActiveSheetIndex(0)
								->setCellValue('AL'.$line, $Course['course_name'])
								->setCellValue('AM'.$line, $Course['user_course_status'] ? 'Complete' : 'Incomplete')
							;
						}
					}
					
					$PHPXlsx->setActiveSheetIndex(0)
						->setCellValue('AP'.$line, $globalTime)//Total time en heures
						->setCellValue('AQ'.$line, $lastAccess)
					;
					$line++;
				}
			}
			...
			header('Content-Disposition: attachment;filename="k&n.xlsx"');
