...
class Xlsx extends Export{
	...
	function main(){
		$dbinstancename = defined('DEVEL') ? 'ynytest' : 'ynynewlms';
		if (!empty($_REQUEST['instance'])) $dbinstancename = $_REQUEST['instance'];
		
		//Retrieving and sorting data
		$this->_view->data = array();
		$dbdata = $this->retrieveBnpData($this->getDb($dbinstancename));
		if (!empty($dbdata)){
			...
			foreach ($this->_view->data as $uid=>$User){
				...
				foreach ($User['learning_plans'] as $path_id=>$LP){
					$PHPXlsx->setActiveSheetIndex(0)
						...
						->setCellValue('E'.$line, '')//Date de début de parcours
						->setCellValue('F'.$line, '')//Date de fin de parcours
					;
